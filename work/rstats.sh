...
echo "\"wireless_ip\"=\"`ifconfig | grep -A 4 en1 | grep inet\ | awk '{print $2}'`\"" >> $LOGFILE
echo "\"wired_ip\"=\"`ifconfig | grep -A 4 en0 | grep inet\ | awk '{print $2}'`\"" >> $LOGFILE
